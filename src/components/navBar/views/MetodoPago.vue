  ...
  <div class="MetodosContainer">
    ...
    <div class="MetodosCheckoutContainer">
      <div style="display: flex; flex-direction: column">
        <p
          style="position: relative; margin-left: auto; font-size: 18px"
          class="MetodoTotal"
        >
          Total:
        </p>
  ...
  methods: {
    ...
    addMetodoPago() {
      ...
      this.datosMetodosPago.forEach((pago) => {
        this.montoPagado = (
          parseFloat(this.montoPagado) + parseFloat(pago.monto)
        ).toFixed(2);
      });
    },
    eliminarPago(index) {
      this.montoPagado = (
        parseFloat(this.montoPagado) -
        parseFloat(this.datosMetodosPago[index].monto)
      ).toFixed(2);
    ...
    checkout() {
      ...
      this.generarPDF({
        ...
        tipoIdentificacion: this.tipoDocumentoCliente,
        identificacion: this.documentoCliente,
    ...
    generarPDF({
      ...
      const nextText = [
        ...
        {
          text: `MONTO NETO:                                        ref. ${total_preIVA.toFixed(
            2
          )}`,
          x: 10,
        },
        {
          text: `IVA:                                               ref. ${total_iva.toFixed(
            2
          )}`,
          x: 10,
        },
        {
          text: `MONTO TOTAL:                                       ref. ${(
            total_preIVA + total_iva
          ).toFixed(2)}`,
          x: 10,
        },
        {
